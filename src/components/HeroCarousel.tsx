...
export default function HeroCarousel({ onScrollClick }: HeroCarouselProps) {
  ...
  return (
    <div className="relative w-full h-full flex flex-col items-center justify-center text-center px-6 overflow-hidden bg-[#000000]">
      {/* Video Background - Double Buffer Approach */}
      <div className="absolute inset-0 w-full h-full z-0 overflow-hidden pointer-events-none">
        {slides.map((slide, index) => {
          // Only render the current slide and the next slide to save resources
            ...
            >
              <div className="absolute top-1/2 left-1/2 w-[300%] h-[300%] -translate-x-1/2 -translate-y-1/2 pointer-events-none">
                <iframe
                  ...
                  src={`https://www.youtube.com/embed/${slide.videoId}?autoplay=1&mute=1&controls=0&loop=1&playlist=${slide.videoId}&showinfo=0&rel=0&iv_load_policy=3&disablekb=1&modestbranding=1&start=2`}
      ...
      <div className="absolute inset-0 bg-[radial-gradient(circle_at_center,_rgba(0,0,0,0.7)_0%,_transparent_70%)] z-[1]" />
          ...
          {slides.map((_, index) => (
            <button
              ...
              className={`h-[2px] w-8 md:w-12 rounded-full transition-all duration-300 ${
                currentSlide === index ? 'bg-gradient-to-r from-[#C9A84C] to-[#E5C05C]' : 'bg-white/30 hover:bg-white/50'
              }`}
