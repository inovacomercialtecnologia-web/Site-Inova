...
const FlowSvg: React.FC<{ accent: string }> = ({ accent }) => (
  <svg className="absolute inset-0 w-full h-full" preserveAspectRatio="none" viewBox="0 0 400 220">
    ...
    <line x1="100" y1="110" x2="165" y2="110" stroke={accent} strokeWidth="1.5" opacity="0.35" />
...
const ALL_CASES: CaseData[] = [
  {
    ...
    problem: 'Uma distribuidora com mais de 40 motoristas gerenciava toda a operação de rotas e entregas via grupos de WhatsApp e planilhas compartilhadas. Sem visibilidade em tempo real, sem histórico de ocorrências e sem rastreamento de carga.',
  ...
  },
  {
    ...
    problem: 'Empresa com alto volume de leads diários no WhatsApp e site. O time comercial respondia manualmente todos os contatos, sem critério de priorização. Leads com alta intenção de compra aguardavam horas por uma resposta.',
...
function CaseModal({ c, onClose }: { c: CaseData; onClose: () => void }) {
  ...
  return (
    ...
    >
      ...
      >
        {/* Hero area */}
        <div className="relative h-[200px] md:h-[240px] overflow-hidden rounded-t-3xl flex-shrink-0"
          ...
          <div className="absolute bottom-0 left-0 right-0 p-4 sm:p-6 md:p-8
            ...
            <p className="text-[11px] font-bold uppercase tracking-[0.3em] mb-2" style={{ color: c.accent }}>
              {c.tag} · {c.segment}
            </p>
...
export default function CasesSection() {
  ...
  return (
    <section className="py-20 md:py-28 bg-[#000000] px-6 md:px-12 lg:px-24 relative overflow-hidden">
        ...
        <div className="flex flex-col md:flex-row md:items-end md:justify-between gap-4 mb-12 md:mb-16">
          <div>
            <motion.p
              ...
              className="text-[#C9A84C] text-[10px] md:text-xs font-medium uppercase tracking-[0.3em] mb-3
          ...
          >
            <Link
              to="/portfolio"
              className="inline-flex items-center gap-2 text-sm text-[#C9A84C] font-medium
                         hover:text-[#E5C05C] transition-colors duration-300 group"
            >
              Ver todos os cases
      ...
      {/* Modal */}
      <AnimatePresence>
        {selected !== null && selected < ALL_CASES.length && (
          <CaseModal c={ALL_CASES[selected]} onClose={() => setSelected(null)} />
        )}
      </AnimatePresence>
    </section>
  );
